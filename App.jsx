...
const BUILD_TAG="v7 \u00b7 designation-first user \u00b7 delete-in-use warnings";
const QUOTES=[
          ...
          {designations.map(d=>{
            ...
            return(
              <div key={d} style={{display:"flex",alignItems:"center",gap:"10px",padding:"10px 12px",borderRadius:"9px",border:"1px solid #e5e7eb",background:"#f9fafb"}}>
                <span style={{fontWeight:"700",fontSize:"14px",color:"#111827"}}>{d}</span>
                {inUse>0&&<span style={{fontSize:"11px",color:"#6b7280"}}>{inUse} user{inUse===1?"":"s"}</span>}
                <button onClick={()=>setPendingDel({kind:"designation",value:d})} title={"Remove "+d} style={{marginLeft:"auto",flexShrink:0,width:"32px",height:"32px",borderRadius:"8px",border:"1px solid #fca5a5",background:"#fef2f2",color:RD,cursor:"pointer",fontSize:"14px"}}><i className="ti ti-trash" aria-hidden/></button>
              </div>
            );
          })}
  ...
  return(
    <div style={{padding:mobile?"8px":"20px",maxWidth:"960px",margin:"0 auto"}}>
      ...
      <Card style={{marginBottom:"14px",borderTop:`4px solid ${NV}`}}>
        ...
        {/* Access level & permissions — advanced, hidden by default. Designation above is the job title; this controls what the user can do. */}
        <div style={{marginTop:"12px",border:"1px solid #e5e7eb",borderRadius:"8px",overflow:"hidden"}}>
          <button onClick={()=>setShowAdvCreate(s=>!s)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 14px",border:"none",background:"#f8fafc",cursor:"pointer",fontSize:"13px",fontWeight:"700",color:"#374151"}}>
            <span><i className="ti ti-lock" style={{marginRight:"7px",color:"#9ca3af"}} aria-hidden/>Access level &amp; permissions <span style={{fontWeight:"400",color:"#9ca3af"}}>· {ROLE_LABELS[roleKey(newUsr.role)]||newUsr.role||"Engineer"}</span></span>
            <i className={"ti "+(showAdvCreate?"ti-chevron-up":"ti-chevron-down")} aria-hidden/>
          </button>
          {showAdvCreate&&(
            <div style={{padding:"12px 14px",borderTop:"1px solid #e5e7eb"}}>
              <p style={{fontSize:"12px",color:"#6b7280",margin:"0 0 10px"}}>This sets what the user can do in the app. Most site staff are <strong>Engineers</strong>. Their designation (above) is only their job title.</p>
              <Grid cols={mobile?"1fr":"1fr 1fr"}>
                <F lbl="Access level">
