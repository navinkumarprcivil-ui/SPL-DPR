...
const BUILD_TAG="v11 \u00b7 one-time permission backfill";
const QUOTES=[
...
function GlobalSettingsPanel({globalLists,setGlobalLists,flash,mobile,users=[]}){
  ...
  const [pendingDel,setPendingDel]=useState(null); // {kind:"dept"|"designation",value} pending delete confirmation
  // Which users currently carry this dept/designation — the "where is it used" list shown before deleting.
  const usersUsing=(kind,value)=>users.filter(u=>kind==="dept"?(u.dept||"")===value:(u.designation||"")===value).map(u=>u.name).filter(Boolean);
  ...
  // Actually remove after confirmation. Depts/designations are user-profile values, so "where used" = the users carrying them.
  function confirmDeleteListItem(){
    if(!pendingDel)return;
    const {kind,value}=pendingDel;
    if(kind==="dept")persistGlobalList("depts",depts.filter(x=>x!==value),"🗑 Department \""+value+"\" removed");
    else persistGlobalList("designations",designations.filter(x=>x!==value),"🗑 Designation \""+value+"\" removed");
    setPendingDel(null);
  }
    ...
    <div style={{padding:mobile?"8px":"20px",maxWidth:"680px",margin:"0 auto",display:"flex",flexDirection:"column",gap:"14px"}}>
      ...
      <Card>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"6px"}}>
          <div style={{fontWeight:"700",fontSize:"15px",color:NV}}>🎭 Designations</div>
          <span style={{fontSize:"12px",color:"#9ca3af"}}>{roles.length} designation{roles.length===1?"":"s"}</span>
        </div>
        <p style={{fontSize:"13px",color:"#6b7280",marginBottom:"12px",marginTop:0}}>These appear in the <strong>Designation</strong> dropdown when creating or editing users, and flow through to Manage Engineers &amp; reports. Each designation also sets what the user can do — the built-in ones (Engineer, Incharge, Management, Admin) carry preset permissions; any custom designation you add behaves like an Engineer until you tick extra permissions on that user.</p>
        {/* Add a designation */}
        <div style={{display:"flex",gap:"8px",marginBottom:"14px"}}>
          <Inp value={newRole} onChange={e=>setNewRole(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")addRole();}} placeholder="New designation — e.g. Deputy Manager"/>
          ...
          {depts.map(d=>{
            ...
            return(
              <div key={d} style={{display:"flex",alignItems:"center",gap:"10px",padding:"10px 12px",borderRadius:"9px",border:"1px solid #e5e7eb",background:"#f9fafb"}}>
                ...
                {inUse>0&&<span style={{fontSize:"11px",color:"#6b7280"}}>{inUse} user{inUse===1?"":"s"}</span>}
                <button onClick={()=>setPendingDel({kind:"dept",value:d})} title={"Remove "+d} style={{marginLeft:"auto",flexShrink:0,width:"32px",height:"32px",borderRadius:"8px",border:"1px solid #fca5a5",background:"#fef2f2",color:RD,cursor:"pointer",fontSize:"14px"}}><i className="ti ti-trash" aria-hidden/></button>
              </div>
            );
          })}
        ...
        <div onClick={()=>setDelRole(null)} style={{position:"fixed",inset:0,background:"rgba(15,23,42,.55)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:2000}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"14px",width:"100%",maxWidth:"400px",overflow:"hidden",boxShadow:"0 30px 60px -20px rgba(0,0,0,.4)"}}>
            ...
            <div style={{padding:"20px"}}>
              ...
              {inUse>0&&(()=>{const names=users.filter(u=>roleKey(u.role)===roleKey(delRole)).map(u=>u.name).filter(Boolean);return(
                <div style={{fontSize:"13px",color:"#92400e",background:"#fffbeb",border:"1px solid #fde68a",borderRadius:"8px",padding:"10px 12px",marginBottom:"12px"}}>
                  <div style={{fontWeight:"700",marginBottom:"4px"}}>⚠️ In use by {inUse} user{inUse===1?"":"s"}</div>
                  <div style={{color:"#78350f"}}>{names.slice(0,8).join(", ")}{names.length>8?" +"+(names.length-8)+" more":""}</div>
                  <div style={{marginTop:"6px",fontSize:"12px",color:"#a16207"}}>They keep this role, but you won't be able to pick it again for new users until you re-add it.</div>
                </div>
              );})()}
      ...
      {/* Delete-department / delete-designation confirmation — shows where the value is used before removing */}
      {pendingDel&&(()=>{const names=usersUsing(pendingDel.kind,pendingDel.value);const label=pendingDel.kind==="dept"?"Department":"Designation";return(
        <div onClick={()=>setPendingDel(null)} style={{position:"fixed",inset:0,background:"rgba(15,23,42,.55)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",zIndex:2000}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"14px",width:"100%",maxWidth:"420px",overflow:"hidden",boxShadow:"0 30px 60px -20px rgba(0,0,0,.4)"}}>
            <div style={{padding:"16px 20px",background:"#fef2f2",borderBottom:"1px solid #fecaca",display:"flex",alignItems:"center",gap:"10px"}}>
              <i className="ti ti-alert-triangle" style={{color:RD,fontSize:"20px"}} aria-hidden/>
              <div style={{fontWeight:"800",fontSize:"15px",color:"#991b1b"}}>Remove {label}</div>
            </div>
            <div style={{padding:"20px"}}>
              <div style={{fontSize:"14px",color:"#374151",marginBottom:"12px"}}>Remove the {label.toLowerCase()} <strong>{pendingDel.value}</strong> from the list?</div>
              {names.length>0?(
                <div style={{fontSize:"13px",color:"#92400e",background:"#fffbeb",border:"1px solid #fde68a",borderRadius:"8px",padding:"10px 12px",marginBottom:"12px"}}>
                  <div style={{fontWeight:"700",marginBottom:"5px"}}>⚠️ Currently used by {names.length} user{names.length===1?"":"s"}:</div>
                  <div style={{color:"#78350f",lineHeight:"1.5"}}>{names.slice(0,10).join(", ")}{names.length>10?" +"+(names.length-10)+" more":""}</div>
                  <div style={{marginTop:"7px",fontSize:"12px",color:"#a16207"}}>Their {label.toLowerCase()} (and how it shows in Manage Engineers &amp; reports) stays as-is, but you won't be able to pick it for other users until you re-add it.</div>
                </div>
              ):(
                <div style={{fontSize:"13px",color:"#166534",background:"#f0fdf4",border:"1px solid #86efac",borderRadius:"8px",padding:"10px 12px",marginBottom:"12px"}}>✅ Not used by any user — safe to remove.</div>
              )}
              <div style={{display:"flex",gap:"10px",marginTop:"4px"}}>
                <button onClick={()=>setPendingDel(null)} style={{flex:1,padding:"11px",borderRadius:"9px",border:"1.5px solid #d1d5db",background:"#fff",color:"#475569",fontWeight:"700",cursor:"pointer",fontSize:"14px"}}>Cancel</button>
                <button onClick={confirmDeleteListItem} style={{flex:1,padding:"11px",borderRadius:"9px",border:"none",background:RD,color:"#fff",fontWeight:"800",cursor:"pointer",fontSize:"14px"}}>Remove</button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div style={{padding:mobile?"8px":"20px",maxWidth:"960px",margin:"0 auto"}}>
      ...
      <Card style={{marginBottom:"14px",borderTop:`4px solid ${NV}`}}>
        ...
        <Grid cols={mobile?"1fr":"1fr 1fr 1fr"}>
          ...
          <F lbl="Designation">
            <select value={newUsr.role} onChange={v=>{const r=v.target.value;setNewUsr(p=>({...p,role:r}));}} style={{width:"100%",boxSizing:"border-box",padding:"11px 12px",borderRadius:"8px",border:"1.5px solid #d1d5db",fontSize:"15px",background:"#fff"}}>
        ...
        {/* Permissions — selectable per user, independent of designation. Defaults to Fill only. */}
        <div style={{marginTop:"12px",padding:"12px",background:"#f8fafc",borderRadius:"8px",border:"1px solid #e5e7eb"}}>
          <div style={{fontWeight:"700",fontSize:"12px",color:"#374151",marginBottom:"4px",textTransform:"uppercase",letterSpacing:"0.05em"}}>Permissions</div>
          <p style={{fontSize:"12px",color:"#6b7280",margin:"0 0 8px"}}>What this user is allowed to do — independent of their designation. <strong>Fill DPR</strong> is on by default; tick or untick the rest as needed.</p>
              ...
              <div key={u.id} style={{border:`2px solid ${isEditing?AM:"#e5e7eb"}`,borderRadius:"12px",overflow:"hidden"}}>
                ...
                {isEditing&&(
                  <div style={{padding:"12px 14px",borderTop:"1px solid #fde68a"}}>
                    ...
                    {/* Department & Designation — Designation is the same value as the role chip above (drives permissions) */}
                    <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:"10px",marginBottom:"12px"}}>
                      ...
                      <div>
                        ...
                        <select value={ed.role||u.role} onChange={e=>setEditData(p=>({...p,role:e.target.value}))} style={{width:"100%",boxSizing:"border-box",padding:"9px 10px",borderRadius:"7px",border:"1.5px solid #d1d5db",fontSize:"13px",background:"#fff"}}>
                          {roleOpts.map(r=><option key={r} value={r}>{ROLE_LABELS[roleKey(r)]||r}</option>)}
                        </select>
                      </div>
                    </div>
...
function App(){
  ...
  // Designation is the user's role value (Role and Designation are one and the same).
  const userDesignation=u=>ROLE_LABELS[roleKey(u.role)]||u.role||u.designation||"";
  const engineers=engineersRaw.map(e=>{
    const u=users.find(x=>x.id===e.id||(x.name||"").trim().toLowerCase()===(e.name||"").trim().toLowerCase());
    return u?{...e,dept:u.dept||e.dept||"",designation:userDesignation(u)||e.designation||""}:e;
  });
  ...
  // Older users had no explicit `caps` and relied on their designation to
  // derive permissions. Now that permissions are selectable per user, pin
  // each such user's *current effective* permissions as explicit caps so
  // nothing changes for them and future edits start from a real value.
  // Runs once per session, only for an admin who can write to users.
  const backfilledCapsRef=useRef(false);
  useEffect(()=>{
    if(backfilledCapsRef.current) return;
    if(!user) return;
    const meCaps=user.caps||ROLE_CAPS[roleKey(user.role)]||ROLE_CAPS.engineer;
    if(!(roleKey(user.role)==="admin"||meCaps.users)) return; // need write access to users
    const missing=users.filter(u=>u&&u.id&&!u.caps);
    if(missing.length===0) return;
    backfilledCapsRef.current=true;
    missing.forEach(u=>{
      const eff=ROLE_CAPS[roleKey(u.role)]||ROLE_CAPS.engineer;
      rtdbPatch('users/'+u.id,{caps:{...eff}}).catch(e=>console.error('caps backfill',u.id,e.message));
    });
    flash("🔐 Pinned permissions for "+missing.length+" existing user"+(missing.length===1?"":"s"));
  },[user,users]);
    ...
    <div style={{fontFamily:"var(--font-sans)",background:"#f8fafc",minHeight:"100vh",paddingBottom:CONTENT_PAD,paddingLeft:mobile?0:"236px"}}>
      ...
      {view==="manage"&&caps.manage&&(
        <div>
          ...
          {(()=>{
            // List everyone assigned to this project who is allowed to fill a DPR (Fill permission on), regardless of designation.
            const canFill=u=>!!(u.caps||ROLE_CAPS[roleKey(u.role)]||ROLE_CAPS.engineer).fill;
            const projEngUsers=users.filter(u=>userAssignedTo(u,activeProject?.id)&&!isSuperAdminUser(u)&&canFill(u));
              ...
              <>
                {noIncharge.length>0&&(
                  <div style={{background:"#fffbeb",border:"2px solid #f59e0b",borderRadius:"10px",padding:"12px 16px",marginBottom:"14px",fontSize:"13px",color:"#92400e",fontWeight:"600"}}>
                    ⚠️ {noIncharge.length} {noIncharge.length>1?"users have":"user has"} no incharge assigned: {noIncharge.map(u=>u.name).join(", ")}
                  </div>
                )}
                {projEngUsers.length===0?(
                  <Card style={{textAlign:"center",padding:"3rem",color:"#9ca3af"}}>
                    <div style={{fontSize:"40px",marginBottom:"8px"}}>👷</div>
                    <div style={{fontWeight:"700",fontSize:"15px",marginBottom:"4px"}}>No site team members yet</div>
                    <div style={{fontSize:"13px",marginBottom:"16px"}}>Go to Global Admin → Users, assign users to this project, and give them the <strong>Fill DPR</strong> permission.</div>
                  </Card>
                ):(
                  <Card>
                    <div style={SH}>
                      <span>Project Site Team ({projEngUsers.length})</span>
                      <span style={{fontSize:"11px",color:"#9ca3af",fontWeight:"400",textTransform:"none"}}>Everyone assigned to this project who can fill a DPR (based on their permissions), whatever their designation. Name, department &amp; designation come from the user's profile. Set each person's incharge below.</span>
                    </div>
                    <div style={{overflowX:"auto"}}>
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:"13px"}}>
                        ...
                        <tbody>
                          {projEngUsers.map(u=>{
                            ...
                            const designation=userDesignation(u)||er.designation||"";
