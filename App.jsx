...
const BUILD_TAG="v9 \u00b7 project team = all assigned users";
const QUOTES=[
...
function UsersPanel({users,projects,mobile,newUsr,setNewUsr,addUser,reassignUser,saveUserCaps,saveUserFields,flash,globalLists,user}){
  ...
  const [showAdvCreate,setShowAdvCreate]=useState(false); // reveal access-level & permissions on Create form
  ...
  return(
    <div style={{padding:mobile?"8px":"20px",maxWidth:"960px",margin:"0 auto"}}>
      ...
      <Card style={{marginBottom:"14px",borderTop:`4px solid ${NV}`}}>
        ...
        {/* Permissions — advanced, hidden by default. Auto-derived from the Designation above; override only if needed. */}
        <div style={{marginTop:"12px",border:"1px solid #e5e7eb",borderRadius:"8px",overflow:"hidden"}}>
          <button onClick={()=>setShowAdvCreate(s=>!s)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 14px",border:"none",background:"#f8fafc",cursor:"pointer",fontSize:"13px",fontWeight:"700",color:"#374151"}}>
            <span><i className="ti ti-lock" style={{marginRight:"7px",color:"#9ca3af"}} aria-hidden/>Permissions <span style={{fontWeight:"400",color:"#9ca3af"}}>· auto-set from designation</span></span>
            <i className={"ti "+(showAdvCreate?"ti-chevron-up":"ti-chevron-down")} aria-hidden/>
          </button>
          {showAdvCreate&&(
            <div style={{padding:"12px 14px",borderTop:"1px solid #e5e7eb"}}>
              <p style={{fontSize:"12px",color:"#6b7280",margin:"0 0 10px"}}>Permissions are set automatically from the chosen <strong>Designation</strong>. Adjust the checkboxes below only if this user needs something different.</p>
        ...
        {/* Permissions — advanced, revealed with Access level. Auto-derived from the chosen access level; override only if needed. */}
        {showAdvCreate&&(
        <div style={{marginTop:"12px",padding:"12px",background:"#f8fafc",borderRadius:"8px",border:"1px solid #e5e7eb"}}>
          <div style={{fontWeight:"700",fontSize:"12px",color:"#374151",marginBottom:"8px",textTransform:"uppercase",letterSpacing:"0.05em"}}>Permissions <span style={{fontWeight:"400",textTransform:"none",color:"#9ca3af"}}>— auto-set from access level; override if needed</span></div>
                  ...
                  <div style={{padding:"12px 14px",borderTop:"1px solid #fde68a"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:"10px",marginBottom:"12px"}}>
                      ...
                      <div>
                        ...
                        <select value={ed.role||u.role} onChange={e=>setEditData(p=>({...p,role:e.target.value,caps:ROLE_CAPS[roleKey(e.target.value)]||ROLE_CAPS.engineer}))} style={{width:"100%",boxSizing:"border-box",padding:"9px 10px",borderRadius:"7px",border:"1.5px solid #d1d5db",fontSize:"13px",background:"#fff"}}>
                          {roleOpts.map(r=><option key={r} value={r}>{ROLE_LABELS[roleKey(r)]||r}</option>)}
                        </select>
                      </div>
                    </div>
    ...
    <div style={{fontFamily:"var(--font-sans)",background:"#f8fafc",minHeight:"100vh",paddingBottom:CONTENT_PAD,paddingLeft:mobile?0:"236px"}}>
      ...
      {view==="manage"&&caps.manage&&(
        <div>
          ...
          {(()=>{
            // Everyone assigned to this project can fill DPRs regardless of role, so list them all here (not just engineers).
            const projEngUsers=users.filter(u=>userAssignedTo(u,activeProject?.id)&&!isSuperAdminUser(u));
              ...
              <>
                ...
                {projEngUsers.length===0?(
                  <Card style={{textAlign:"center",padding:"3rem",color:"#9ca3af"}}>
                    ...
                    <div style={{fontWeight:"700",fontSize:"15px",marginBottom:"4px"}}>No users assigned to this project</div>
                    <div style={{fontSize:"13px",marginBottom:"16px"}}>Go to Global Admin → Users and assign users to this project.</div>
                  </Card>
                ):(
                  <Card>
                    <div style={SH}>
                      <span>Project Team ({projEngUsers.length})</span>
                      <span style={{fontSize:"11px",color:"#9ca3af",fontWeight:"400",textTransform:"none"}}>All users assigned to this project (any role can fill a DPR). Name, department &amp; designation come from the user's profile. Set each person's incharge below.</span>
                    </div>
                    <div style={{overflowX:"auto"}}>
